...
public class ServletFormulaire extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if (request.getParameter("btnEnvoyer") != null) {
			...
			//Normalement le serveur trouve la session utilisateur à l'aide de l'id de session déposé sur le poste
			//de l'utilisateur dans un cookie
			//Si l'utilisateur interdit l'usage des cookies, l'ID ne peut plus être recupéré et la session qui existe ne peut plus être retrouvée
			//Pour éviter ce genre de situation, Java EE propose la technique de réécriture d'URL
			response.sendRedirect(response.encodeRedirectURL("/08_ReecritureURL/compterendu.do"));
			return;
